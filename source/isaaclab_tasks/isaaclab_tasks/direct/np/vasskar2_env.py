...
from .np_env_cfg import OBS_DIM_CFG, STATE_DIM_CFG, FrankaVasskar2Cfg
...
class FrankaVasskar2Env(DirectRLEnv):
    cfg: FrankaVasskar2Cfg

    def __init__(self, cfg: FrankaVasskar2Cfg, render_mode: str | None = None, **kwargs):
    ...
    def _setup_scene(self):
        ...
        if self.cfg_task.task_idx == 4:
            ...
            self._screw2 = RigidObject(self.cfg_task.screw2)
            self._screw3 = RigidObject(self.cfg_task.screw3)
            self._screw4 = RigidObject(self.cfg_task.screw4)
            self._held_asset = self._screw4
    ...
    def _create_screw_joint(self,connection_idx):
        ...
        # Compute relative joint pose
        rel_mat = connection_cfg.pose_to_base
        pos1 = Gf.Vec3f([float(rel_mat[0, 3]), float(rel_mat[1, 3]), float(rel_mat[2, 3])])
        rot1q = torch_utils.rot_matrices_to_quats(torch.tensor(rel_mat[:3, :3]))
        rot1 = Gf.Quatf(float(rot1q[0]), float(rot1q[1]), float(rot1q[2]), float(rot1q[3]))

        # Zero velocity of held and fixed assets
        ...
        # Create D6 Joint allowing Z-axis rotation and translation
    ...
    def get_handheld_asset_relative_pose(self):
        ...
        if self.cfg_task.name == "vasskar_assembly" and self.cfg_task.task_idx in [1, 2, 3, 4, 5]:
