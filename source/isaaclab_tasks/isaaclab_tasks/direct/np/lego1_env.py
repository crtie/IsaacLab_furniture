...
from .np_env_cfg import OBS_DIM_CFG, STATE_DIM_CFG, FrankaLego1Cfg
...
class FrankaLego1Env(DirectRLEnv):
    cfg: FrankaLego1Cfg

    def __init__(self, cfg: FrankaLego1Cfg, render_mode: str | None = None, **kwargs):
    ...
    def _setup_scene(self):
        ...
        if self.cfg_task.task_idx == 1:
            self._hand = RigidObject(self.cfg_task.hand)
            self._held_asset = self._hand
    ...
    def randomize_initial_state(self, env_ids):
        ...
        if self.cfg_task.task_idx == 1:
            rot_euler = torch.tensor([2.7, 3.1415, -1.5707], device=self.device).repeat(
            self.num_envs, 1
            )
